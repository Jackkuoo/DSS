...
def train(opt):
    ...
    dataloader = torch.utils.data.DataLoader(DogDataset('dataset/images_crop'),
...
def inference(opt):
    ...
    # Load trained models
    discriminator.load_state_dict(torch.load(opt.model_path + '/discriminator.pt'))
    ...
    for j in range(10):
        for i in tqdm(range(0, 1000)):
            save_image(gen_imgs.data[i],
                       "result/images_inference/%d.png" % int(i+(1000*j)),
                       normalize=True)
...
def process_data(opt):
    ...
    # Config dataloader
    root_images = pjoin(opt.data_path, 'images/') #'dataset/images/'
    root_annots = pjoin(opt.data_path, 'annotations/')#'dataset/annotations/'
