...
def computeKeypointsAndDescriptors(image, sigma=1.6, num_intervals=3, assumed_blur=0.5, image_border_width=5):
	...
	#print("keypoints origin",len(keypoints))
	keypoints = removeDuplicateKeypoints(keypoints)
	#print("remove Duplicate",len(keypoints))
	keypoints = convertKeypointsToInputImageSize(keypoints)
	#print("convertKeypointsToInputImageSize",len(keypoints))
...
def findScaleSpaceExtrema(gaussian_images, dog_images, num_intervals, sigma, image_border_width, contrast_threshold=0.13):
...
def isPixelAnExtremum(first_subimage, second_subimage, third_subimage, threshold):
	...
	if abs(center_pixel_value) > threshold:
		if center_pixel_value > 0:
			return all(center_pixel_value >= first_subimage) and all(center_pixel_value >= third_subimage) and all(center_pixel_value >= second_subimage)
...
def repair(img):
	mask = zeros((img.shape[0],img.shape[1],1))
...
def create_panorama(array_of_image,image_width):
	# create_panorama use DivideAndConquer
	if len(array_of_image)==1:
		return array_of_image[0]
	else:
		mid = int(len(array_of_image)/2)
		return SIFT(create_panorama(array_of_image[:mid],image_width),create_panorama(array_of_image[mid:],image_width),image_width)


# In[5]:

def SIFT(image_left,image_right,image_width):
	image_width = int(image_width/2)
	cut_left = image_left[:,:image_left.shape[1]-image_width,:]
	st_left  = image_left[:,image_left.shape[1]-image_width:,:]
	cut_right = image_right[:,image_width:,:]
	st_right  = image_right[:,:image_width,:]
	img_left_gray = cv2.cvtColor(st_left , cv2.COLOR_BGR2GRAY)
	img_right_gray= cv2.cvtColor(st_right, cv2.COLOR_BGR2GRAY)
	# plt.figure()
	# plt.imshow(img_left_gray)
	# plt.show()
	# plt.figure()
	# plt.imshow(img_right_gray)
	# plt.show()
	# sift = cv2.SIFT_create()
	# kp1, des1 = sift.detectAndCompute(img_left_gray,None)
	# kp2, des2 = sift.detectAndCompute(img_right_gray,None)
	kp1, des1 = computeKeypointsAndDescriptors(img_left_gray)
	kp2, des2 = computeKeypointsAndDescriptors(img_right_gray)
	try:
		res = stitch_image(st_left,st_right,kp1,kp2,des1,des2)
	except:
		res = np.concatenate((st_left,st_right),axis = 1)
	res = np.concatenate((cut_left,res),axis = 1)
	res = np.concatenate((res,cut_right),axis = 1)
	res = repair(res)
	# plt.figure()
	# plt.imshow(res)
	# plt.show()
